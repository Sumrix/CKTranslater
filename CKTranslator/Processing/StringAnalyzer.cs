...
namespace CKTranslator.Processing
{
    ...
    public class StringAnalyzer
    {
        ...
        public Event LoadRusFiles(FileContext context)
        {
            ...
            {
                return this.rusLocals.Load(context);
            }
            return this.rusScripts.Load(context);
        }
        ...
        public Event LoadEngFiles(FileContext context)
        {
            if (context.ModFolder == "localisation")
            {
                return this.engLocals.Load(context);
            }
            return this.engScripts.Load(context);
        }
        ...
        public void FilterAndSaveResults()
        {
            ...
            SaveToFile("StringIdentifiers.txt", stringIdentifiers);
            SaveToFile("StringPaths.txt", stringPaths);
        ...
        private void LoadStrings(string fileName, Dictionary<ScriptKey, string> strings)
        {
            ...
            foreach (string line in File.ReadAllLines(@"..\..\..\Data\Strings\" + fileName))
            {
                ...
                List<string> steps = match.Groups[2].Value.Split('.').ToList();
                string value = match.Groups[3].Value;

                strings.Add(
                    new ScriptKey
                    {
                        Path = new Parsing.Path(steps),
