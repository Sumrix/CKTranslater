...
namespace SimilarityEditor
{
    public class LetterSimilarityGrid : DataGridView
    {
        ...
        protected override void OnCellPainting(DataGridViewCellPaintingEventArgs e)
        {
            ...
            bool redHorizongalBorders = false;
            bool redVerticalBorders = false;

            if (this.highlightedCells.Any(p => p.X == e.ColumnIndex && p.Y >= e.RowIndex))
            {
                redVerticalBorders = true;
            }

            if (this.highlightedCells.Any(p => p.X >= e.ColumnIndex && p.Y == e.RowIndex))
            {
                redHorizongalBorders = true;
            }
        ...
        protected override void OnSelectionChanged(EventArgs e)
        {
            ...
            DataGridViewCell cell = this.SelectedCells?.Count > 0 ? this.SelectedCells?[0] : null;
