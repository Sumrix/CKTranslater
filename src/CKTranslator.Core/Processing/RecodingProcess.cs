...
{
    public class RecodingProcess : IModuleProcess
    {
        // Simplifies characters like: Ë = E
        private static readonly byte[][] simpleCharmap =
        {
            new byte[] { 0x88 }, // 0x80: € = €
            new byte[] { 0x81 }, // 0x81
            new byte[] { 0x82 }, // 0x82
        ...
        // Simplifies characters like: Ë = E4
        private static readonly byte[][] wideCharmap =
        {
            new byte[] { 0x88 }, // 0x80: € = €
            new byte[] { 0x81 }, // 0x81
            new byte[] { 0x82 }, // 0x82
        ...
        public bool IsProcessable(Module module) => true;

        public void ProcessDocument(Document document)
        {
            byte[] content = File.ReadAllBytes(document.FullFileName);
            ...
            byte[] resultContent = ArrayPool<byte>.Shared.Rent(length);

            using (MemoryStream mem = new(resultContent))
            {
                foreach (byte b in content)
                {
                    if (b >= 0x80)
                    {
                        byte[] buffer = wideCharmap[b - 0x80];
            ...
            using FileStream stream = File.OpenWrite(document.FullFileName);
            stream.Write(resultContent, 0, length);

            // I hope there will be no memory leak in case of exceptions.
            ArrayPool<byte>.Shared.Return(resultContent);
        }

        public IList<Document> SelectDocuments(Module module) => module.GetModuleFiles();
    }
}